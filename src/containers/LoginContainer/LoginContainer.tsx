...
class RawLoginContainer extends Component<
	LoginContainerProps,
	RawLoginContainerState
> {
	readonly state = {
		login: '',
		password: ''
	...
	render() {
		return (
			<Login onSubmit={this.onSubmitHandler} onChange={this.onHandleChange} />
		);
	}
	...
	private onSubmitHandler = (event: FormEvent<HTMLFormElement>) => {
		...
		this.loggedIn('http://localhost:8888/login').then(() => {
			this.userData('http://localhost:8888/currentUser').then(
